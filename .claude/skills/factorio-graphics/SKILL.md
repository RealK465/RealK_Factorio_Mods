...
**Don't reinvent the parts.** `greeble.py` builds the vocabulary `references/design-language.md` asks for — flanged pipe runs, sagging cables, louvre banks, finned radiators, bolt circles, rivet rows, handwheels, gauge pods, ladders, railings, skid feet, placards — each already bevelled, non-coplanar and normal-correct. Reaching "8–15 distinct kinds of functional detail" is a composition problem once they exist. And call `greeble.legibility(size)` **before** modelling anything small: under ~3 px in play a feature is texture, not geometry, and modelling it costs render time to show nothing.
