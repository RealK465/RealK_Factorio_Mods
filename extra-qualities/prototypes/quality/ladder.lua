...
-- quality effect times this value.
--
-- **These are 2.0 numbers and they are a tenth of the 2.1 file's.** 2.1.7 divided every quality
-- effect by ten and multiplied every next_probability by ten to compensate, so vanilla writes
-- 0.1 here and 1 there, and a four-module assembler reads 1.0 of effect here and 0.1 there.
-- The player-facing result is identical: 0.14 turns a 10% quality effect into a 14% chance of
-- stepping up, exactly as 1.4 does on 2.1. Copying the 2.1 values across would make every step
-- ten times as likely, and nothing would error.
--
-- chain_probability does not exist on 2.0, so there is no skip-a-tier roll to hold down. Where
-- the 2.1 file has to set it explicitly to stop a raised next_probability dragging it along,
-- here it is simply absent.

quality["normal"].next_probability = 0.14
quality["uncommon"].next_probability = 0.13
quality["rare"].next_probability = 0.11
...
quality["legendary"].next_probability = 0.1
