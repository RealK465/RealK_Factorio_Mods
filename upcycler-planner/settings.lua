...
-- settings panel is where a player will actually tick them. A mod may overwrite its own
-- per-player settings, and doing so raises on_runtime_mod_setting_changed exactly as the game's
-- settings menu does (both measured on 2.1.14), so the panel is a second face on one value
