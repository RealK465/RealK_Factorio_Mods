...
- **`quality-assembler-design.md`** — the entity's visual design and, in its *Built* section,
  ...
  numbers against the assembling machine 3, and the two engine facts (`idle_animation` is
  frozen; `pipe_picture` draws on the outside tile). **Read before touching the model, the
  render scripts or the prototype's graphics.** It also sets the mod's house art style, since
  there is no separate art-direction register.
...
- **`deferred.md`** — open questions and parked work: the `legacy/2.0` port, sounds, the
  remnant, the connector check, the gate number still under its band, balance. Read before
