...
- **Only a mod's own source may differ between the two branches.** `CLAUDE.md`, `README.md`,
  ...
  **`chain_probability`, `locomotive_power_multiplier` and `rolling_stock_max_speed_multiplier`
  ...
  else, the sheets included. Four fields differ in the first: `factorio_version`, the `base`
  floor, the `quality` floor and `homepage`. **The version deliberately does not**: it is
  `0.1.0` on both tracks because nothing has shipped on either, and a second `Date: ????`
  section stacked on an unshipped one is exactly what `factorio-changelog` forbids. Three things
